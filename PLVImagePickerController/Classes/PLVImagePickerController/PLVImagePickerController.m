//
//  PLVImagePickerController.m
//  PLVImagePickerController
//
//  Created by 谭真 on 15/12/24.
//  Copyright © 2015年 谭真. All rights reserved.
//  version 3.6.0 - 2021.04.02
//  更多信息，请前往项目的github地址：https://github.com/banchichen/PLVImagePickerController
...
#import "PLVVideoCropController.h"

@interface PLVImagePickerController () {
    NSTimer *_timer;
    UILabel *_tipLabel;
...
/// Default is 4, Use in photos collectionView in PLVPhotoPickerController
/// 默认4列, PLVPhotoPickerController中的照片collectionView
@property (nonatomic, assign) NSInteger columnNumber;
@property (nonatomic, assign) NSInteger HUDTimeoutCount; ///< 超时隐藏HUD计数
@end

@implementation PLVImagePickerController
...
- (void)viewDidLoad {
    ...
    [PLVImageManager manager].shouldFixOrientation = NO;
...
- (void)configBarButtonItemAppearance {
    ...
    if (@available(iOS 9, *)) {
        barItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[PLVImagePickerController class]]];
    } else {
        barItem = [UIBarButtonItem appearanceWhenContainedIn:[PLVImagePickerController class], nil];
    }
    NSMutableDictionary *textAttrs = [NSMutableDictionary dictionary];
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount delegate:(id<PLVImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:4 delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<PLVImagePickerControllerDelegate>)delegate {
    return [self initWithMaxImagesCount:maxImagesCount columnNumber:columnNumber delegate:delegate pushPhotoPickerVc:YES];
}

- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<PLVImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    _pushPhotoPickerVc = pushPhotoPickerVc;
    PLVAlbumPickerController *albumPickerVc = [[PLVAlbumPickerController alloc] init];
    ...
    if (self) {
        ...
        if (![[PLVImageManager manager] authorizationStatusAuthorized]) {
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    PLVPhotoPreviewController *previewVc = [[PLVPhotoPreviewController alloc] init];
...
- (void)setShowSelectedIndex:(BOOL)showSelectedIndex {
    ...
    [PLVImagePickerConfig sharedInstance].showSelectedIndex = showSelectedIndex;
}

- (void)setShowPhotoCannotSelectLayer:(BOOL)showPhotoCannotSelectLayer {
    _showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
    [PLVImagePickerConfig sharedInstance].showPhotoCannotSelectLayer = showPhotoCannotSelectLayer;
}

- (void)setNotScaleImage:(BOOL)notScaleImage {
    _notScaleImage = notScaleImage;
    [PLVImagePickerConfig sharedInstance].notScaleImage = notScaleImage;
}

- (void)setNeedFixComposition:(BOOL)needFixComposition {
    _needFixComposition = needFixComposition;
    [PLVImagePickerConfig sharedInstance].needFixComposition = needFixComposition;
}
...
- (void)observeAuthrizationStatusChange {
    ...
    if ([[PLVImageManager manager] authorizationStatusAuthorized]) {
        ...
        PLVAlbumPickerController *albumPickerVc = (PLVAlbumPickerController *)self.visibleViewController;
        if ([albumPickerVc isKindOfClass:[PLVAlbumPickerController class]]) {
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        PLVPhotoPickerController *photoPickerVc = [[PLVPhotoPickerController alloc] init];
        photoPickerVc.isFirstAppear = YES;
        photoPickerVc.columnNumber = self.columnNumber;
        [[PLVImageManager manager] getCameraRollAlbumWithFetchAssets:NO completion:^(PLVAlbumModel *model) {
...
- (void)setPickerDelegate:(id<PLVImagePickerControllerDelegate>)pickerDelegate {
    _pickerDelegate = pickerDelegate;
    [PLVImageManager manager].pickerDelegate = pickerDelegate;
}
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    [PLVImageManager manager].photoPreviewMaxWidth = _photoPreviewMaxWidth;
}

- (void)setPhotoWidth:(CGFloat)photoWidth {
    _photoWidth = photoWidth;
    [PLVImageManager manager].photoWidth = photoWidth;
}
...
- (void)setSelectedAssets:(NSMutableArray *)selectedAssets {
    ...
    for (PHAsset *asset in selectedAssets) {
        PLVAssetModel *model = [PLVAssetModel modelWithAsset:asset type:[[PLVImageManager manager] getAssetType:asset]];
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [PLVImagePickerConfig sharedInstance].allowPickingImage = allowPickingImage;
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [PLVImagePickerConfig sharedInstance].allowPickingVideo = allowPickingVideo;
...
- (void)setPreferredLanguage:(NSString *)preferredLanguage {
    ...
    [PLVImagePickerConfig sharedInstance].preferredLanguage = preferredLanguage;
    [self configDefaultBtnTitle];
}

- (void)setLanguageBundle:(NSBundle *)languageBundle {
    _languageBundle = languageBundle;
    [PLVImagePickerConfig sharedInstance].languageBundle = languageBundle;
    [self configDefaultBtnTitle];
}
...
- (void)setSortAscendingByModificationDate:(BOOL)sortAscendingByModificationDate {
    ...
    [PLVImageManager manager].sortAscendingByModificationDate = sortAscendingByModificationDate;
}
...
- (void)addSelectedModel:(PLVAssetModel *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.asset.localIdentifier];
}

- (void)removeSelectedModel:(PLVAssetModel *)model {
...
- (UIInterfaceOrientationMask)supportedInterfaceOrientations {
    if ([self.topViewController isKindOfClass:PLVVideoPlayerController.class] && self.topViewController.presentedViewController) {
...
@interface PLVAlbumPickerController ()<UITableViewDataSource, UITableViewDelegate, PHPhotoLibraryChangeObserver> {
    UITableView *_tableView;
}
@property (nonatomic, strong) NSMutableArray *albumArr;
@end

@implementation PLVAlbumPickerController
...
- (void)viewDidLoad {
    ...
    [PLVCommonTools configBarButtonItem:cancelItem tzImagePickerVc:imagePickerVc];
...
- (void)configTableView {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        [[PLVImageManager manager] getAllAlbumsWithFetchAssets:!self.isFirstAppear completion:^(NSArray<PLVAlbumModel *> *models) {
            dispatch_async(dispatch_get_main_queue(), ^{
                self->_albumArr = [NSMutableArray arrayWithArray:models];
                for (PLVAlbumModel *albumModel in self->_albumArr) {
                ...
                if (!self->_tableView) {
                    ...
                    [self->_tableView registerClass:[PLVAlbumCell class] forCellReuseIdentifier:@"PLVAlbumCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    PLVAlbumCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PLVAlbumCell"];
    if (@available(iOS 13.0, *)) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    PLVPhotoPickerController *photoPickerVc = [[PLVPhotoPickerController alloc] init];
    photoPickerVc.columnNumber = self.columnNumber;
    PLVAlbumModel *model = _albumArr[indexPath.row];
    photoPickerVc.model = model;
...
+ (void)configBarButtonItem:(UIBarButtonItem *)item tzImagePickerVc:(PLVImagePickerController *)tzImagePickerVc {
...
@implementation PLVImagePickerConfig

+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static PLVImagePickerConfig *config = nil;
    dispatch_once(&onceToken, ^{
        if (config == nil) {
            config = [[PLVImagePickerConfig alloc] init];
