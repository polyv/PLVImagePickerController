...
@implementation PLVPhotoPreviewCell

- (void)configSubviews {
    self.previewView = [[PLVPhotoPreviewView alloc] initWithFrame:CGRectZero];
...
- (void)setModel:(PLVAssetModel *)model {
...
@interface PLVPhotoPreviewView ()<UIScrollViewDelegate>
@property (assign, nonatomic) BOOL isRequestingGIF;
@end

@implementation PLVPhotoPreviewView
...
- (void)configProgressView {
    _progressView = [[PLVProgressView alloc] init];
...
- (void)setModel:(PLVAssetModel *)model {
    ...
    if (model.type == PLVAssetModelMediaTypePhotoGif) {
        // 先显示缩略图
        [[PLVImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            if (photo) {
            ...
            [[PLVImageManager manager] getOriginalPhotoDataWithAsset:model.asset progressHandler:^(double progress, NSError *error, BOOL *stop, NSDictionary *info) {
                progress = progress > 0.02 ? progress : 0.02;
                dispatch_async(dispatch_get_main_queue(), ^{
                    BOOL iCloudSyncFailed = [PLVCommonTools isICloudSyncError:error];
...
#ifdef DEBUG
                NSLog(@"[PLVImagePickerController] getOriginalPhotoDataWithAsset:%f error:%@", progress, error);
#endif
            } completion:^(NSData *data, NSDictionary *info, BOOL isDegraded) {
                if (!isDegraded) {
                    self.isRequestingGIF = NO;
                    self.progressView.hidden = YES;
                    if ([PLVImagePickerConfig sharedInstance].gifImagePlayBlock) {
                        [PLVImagePickerConfig sharedInstance].gifImagePlayBlock(self, self.imageView, data, info);
                    } else {
...
- (void)setAsset:(PHAsset *)asset {
    ...
    _asset = asset;
    self.imageRequestID = [[PLVImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        BOOL iCloudSyncFailed = !photo && [PLVCommonTools isICloudSyncError:info[PHImageErrorKey]];
        self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
        self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
        if (self.iCloudSyncFailedHandle) {
...
- (void)layoutSubviews {
    ...
    _iCloudErrorIcon.frame = CGRectMake(20, [PLVCommonTools tz_statusBarHeight] + 44 + 10, 28, 28);
    _iCloudErrorLabel.frame = CGRectMake(53, [PLVCommonTools tz_statusBarHeight] + 44 + 10, self.tz_width - 63, 28);
}
...
@implementation PLVVideoPreviewCell
...
- (void)configMoviePlayer {
    ...
    if (self.model && self.model.asset) {
        [[PLVImageManager manager] getPhotoWithAsset:self.model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            BOOL iCloudSyncFailed = !photo && [PLVCommonTools isICloudSyncError:info[PHImageErrorKey]];
            self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
            self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
            if (self.iCloudSyncFailedHandle) {
        ...
        [[PLVImageManager manager] getVideoWithAsset:self.model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
            dispatch_async(dispatch_get_main_queue(), ^{
                BOOL iCloudSyncFailed = !playerItem && [PLVCommonTools isICloudSyncError:info[PHImageErrorKey]];
                self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
                self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
                if (self.iCloudSyncFailedHandle) {
                    self.iCloudSyncFailedHandle(self.model.asset, iCloudSyncFailed);
...
- (void)playButtonClick {
    ...
    if (_player.rate == 0.0f) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"PLV_VIDEO_PLAY_NOTIFICATION" object:_player];
...
@implementation PLVGifPreviewCell
