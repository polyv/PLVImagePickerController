...
@interface PLVImageManager ()
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
@end

@implementation PLVImageManager

CGSize AssetGridThumbnailSize;
CGFloat PLVScreenWidth;
CGFloat PLVScreenScale;

static PLVImageManager *manager;
...
- (void)setPhotoWidth:(CGFloat)photoWidth {
    ...
    PLVScreenWidth = photoWidth / 2;
}

- (void)setColumnNumber:(NSInteger)columnNumber {
    [self configPLVScreenWidth];

    _columnNumber = columnNumber;
    CGFloat margin = 4;
    CGFloat itemWH = (PLVScreenWidth - 2 * margin - 4) / columnNumber - margin;
    AssetGridThumbnailSize = CGSizeMake(itemWH * PLVScreenScale, itemWH * PLVScreenScale);
}

- (void)configPLVScreenWidth {
    PLVScreenWidth = [UIScreen mainScreen].bounds.size.width;
    // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
    PLVScreenScale = 2.0;
    if (PLVScreenWidth > 700) {
        PLVScreenScale = 1.5;
    }
}
...
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(PLVAlbumModel *model))completion {
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbumWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(PLVAlbumModel *model))completion {
    __block PLVAlbumModel *model;
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
...
- (void)getAllAlbums:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage needFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<PLVAlbumModel *> *))completion {
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAllAlbumsWithFetchAssets:(BOOL)needFetchAssets completion:(void (^)(NSArray<PLVAlbumModel *> *))completion {
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(PHFetchResult *)result allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(NSArray<PLVAssetModel *> *))completion {
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetsFromFetchResult:(PHFetchResult *)result completion:(void (^)(NSArray<PLVAssetModel *> *))completion {
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
    NSMutableArray *photoArr = [NSMutableArray array];
    [result enumerateObjectsUsingBlock:^(PHAsset *asset, NSUInteger idx, BOOL * _Nonnull stop) {
        PLVAssetModel *model = [self assetModelWithAsset:asset allowPickingVideo:config.allowPickingVideo allowPickingImage:config.allowPickingImage];
        if (model) {
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage completion:(void (^)(PLVAssetModel *))completion {
    PLVImagePickerConfig *config = [PLVImagePickerConfig sharedInstance];
    config.allowPickingVideo = allowPickingVideo;
    config.allowPickingImage = allowPickingImage;
...
- (void)getAssetFromFetchResult:(PHFetchResult *)result atIndex:(NSInteger)index completion:(void (^)(PLVAssetModel *))completion {
    PHAsset *asset;
    @try {
        asset = result[index];
    }
...
- (PLVAssetModel *)assetModelWithAsset:(PHAsset *)asset allowPickingVideo:(BOOL)allowPickingVideo allowPickingImage:(BOOL)allowPickingImage {
    BOOL canSelect = YES;
    ...
    PLVAssetModel *model;
    PLVAssetModelMediaType type = [self getAssetType:asset];
    if (!allowPickingVideo && type == PLVAssetModelMediaTypeVideo) return nil;
    if (!allowPickingImage && type == PLVAssetModelMediaTypePhoto) return nil;
    if (!allowPickingImage && type == PLVAssetModelMediaTypePhotoGif) return nil;
    ...
    NSString *timeLength = type == PLVAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",phAsset.duration] : @"";
    timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
    model = [PLVAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}

- (PLVAssetModelMediaType)getAssetType:(PHAsset *)asset {
    PLVAssetModelMediaType type = PLVAssetModelMediaTypePhoto;
    PHAsset *phAsset = (PHAsset *)asset;
    if (phAsset.mediaType == PHAssetMediaTypeVideo)      type = PLVAssetModelMediaTypeVideo;
    else if (phAsset.mediaType == PHAssetMediaTypeAudio) type = PLVAssetModelMediaTypeAudio;
    else if (phAsset.mediaType == PHAssetMediaTypeImage) {
        if (@available(iOS 9.1, *)) {
            // if (asset.mediaSubtypes == PHAssetMediaSubtypePhotoLive) type = PLVAssetModelMediaTypeLivePhoto;
        }
        // Gif
        if ([[phAsset valueForKey:@"filename"] hasSuffix:@"GIF"]) {
            type = PLVAssetModelMediaTypePhotoGif;
        }
    }
    return type;
}
...
- (void)getPhotosBytesWithArray:(NSArray *)photos completion:(void (^)(NSString *totalBytes))completion {
    ...
    for (NSInteger i = 0; i < photos.count; i++) {
        PLVAssetModel *model = photos[i];
        PHImageRequestOptions *options = [[PHImageRequestOptions alloc] init];
        options.resizeMode = PHImageRequestOptionsResizeModeFast;
        options.networkAccessAllowed = YES;
        if (model.type == PLVAssetModelMediaTypePhotoGif) {
        ...
        [[PHImageManager defaultManager] requestImageDataForAsset:model.asset options:options resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
            if (model.type != PLVAssetModelMediaTypeVideo) dataLength += imageData.length;
            assetCount ++;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGFloat fullScreenWidth = PLVScreenWidth;
...
- (PHImageRequestID)getPhotoWithAsset:(PHAsset *)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    CGSize imageSize;
    if (photoWidth < PLVScreenWidth && photoWidth < _photoPreviewMaxWidth) {
        imageSize = AssetGridThumbnailSize;
    } else {
        PHAsset *phAsset = (PHAsset *)asset;
        CGFloat aspectRatio = phAsset.pixelWidth / (CGFloat)phAsset.pixelHeight;
        CGFloat pixelWidth = photoWidth * PLVScreenScale;
        // 超宽图片
        if (aspectRatio > 1.8) {
...
/// Get postImage / 获取封面图
- (PHImageRequestID)getPostImageWithAlbumModel:(PLVAlbumModel *)model completion:(void (^)(UIImage *))completion {
    ...
    return [[PLVImageManager manager] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset timeRange:(CMTimeRange)timeRange presetName:(NSString *)presetName success:(void (^)(NSString *outputPath))success failure:(void (^)(NSString *errorMessage, NSError *error))failure  {
    ...
    if ([presets containsObject:presetName]) {
        ...
        if ([PLVImagePickerConfig sharedInstance].needFixComposition) {
...
- (PLVAlbumModel *)modelWithResult:(PHFetchResult *)result collection:(PHAssetCollection *)collection isCameraRoll:(BOOL)isCameraRoll needFetchAssets:(BOOL)needFetchAssets options:(PHFetchOptions *)options {
    PLVAlbumModel *model = [[PLVAlbumModel alloc] init];
...
- (PLVAssetModel *)createModelWithAsset:(PHAsset *)asset {
    PLVAssetModelMediaType type = [[PLVImageManager manager] getAssetType:asset];
    NSString *timeLength = type == PLVAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
    timeLength = [[PLVImageManager manager] getNewTimeFromDurationSecond:timeLength.integerValue];
    PLVAssetModel *model = [PLVAssetModel modelWithAsset:asset type:type timeLength:timeLength];
    return model;
}
...
//@implementation PLVSortDescriptor
//
//- (id)reversedSortDescriptor {
//    return [NSNumber numberWithBool:![PLVImageManager manager].sortAscendingByModificationDate];
//}
//
//@end
