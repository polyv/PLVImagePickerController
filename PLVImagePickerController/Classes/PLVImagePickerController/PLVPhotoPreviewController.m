...
#import "PLVImageCropManager.h"

@interface PLVPhotoPreviewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate> {
...
- (void)viewDidLoad {
    ...
    PLVImagePickerController *_tzImagePickerVc = (PLVImagePickerController *)self.navigationController;
...
- (void)configCollectionView {
    ...
    [_collectionView registerClass:[PLVPhotoPreviewCell class] forCellWithReuseIdentifier:@"PLVPhotoPreviewCell"];
    [_collectionView registerClass:[PLVVideoPreviewCell class] forCellWithReuseIdentifier:@"PLVVideoPreviewCell"];
    [_collectionView registerClass:[PLVGifPreviewCell class] forCellWithReuseIdentifier:@"PLVGifPreviewCell"];
...
- (void)configCropView {
    ...
    if (_tzImagePickerVc.maxImagesCount <= 1 && _tzImagePickerVc.allowCrop && _tzImagePickerVc.allowPickingImage) {
        ...
        [PLVImageCropManager overlayClippingWithView:_cropBgView cropRect:_tzImagePickerVc.cropRect containerView:self.view needCircleCrop:_tzImagePickerVc.needCircleCrop];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat statusBarHeight = isFullScreen ? [PLVCommonTools tz_statusBarHeight] : 0;
...
- (void)select:(UIButton *)selectButton {
    PLVImagePickerController *_tzImagePickerVc = (PLVImagePickerController *)self.navigationController;
    PLVAssetModel *model = _models[self.currentIndex];
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            ...
            if (model.type == PLVAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
        ...
        for (PLVAssetModel *model_item in selectedModels) {
            if ([model.asset.localIdentifier isEqualToString:model_item.asset.localIdentifier]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    PLVAssetModel *model = selectedModelsTmp[i];
    ...
    [self refreshNaviBarAndBottomBarState];
    if (model.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:selectButton.imageView.layer type:PLVOscillatoryAnimationToBigger];
    }
    [UIView showOscillatoryAnimationWithLayer:_numberImageView.layer type:PLVOscillatoryAnimationToSmaller];
}

- (void)backButtonClick {
    if (self.navigationController.childViewControllers.count < 2) {
        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
        if ([self.navigationController isKindOfClass: [PLVImagePickerController class]]) {
            PLVImagePickerController *nav = (PLVImagePickerController *)self.navigationController;
...
- (void)doneButtonClick {
    ...
    PLVPhotoPreviewCell *cell = (PLVPhotoPreviewCell *)[_collectionView cellForItemAtIndexPath:indexPath];
    if (_tzImagePickerVc.allowCrop && [cell isKindOfClass:[PLVPhotoPreviewCell class]]) { // 裁剪状态
        _doneButton.enabled = NO;
        [_tzImagePickerVc showProgressHUD];
        UIImage *cropedImage = [PLVImageCropManager cropImageView:cell.previewView.imageView toRect:_tzImagePickerVc.cropRect zoomScale:cell.previewView.scrollView.zoomScale containerView:self.view];
        if (_tzImagePickerVc.needCircleCrop) {
            cropedImage = [PLVImageCropManager circularClipImage:cropedImage];
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PLVAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == PLVAssetModelMediaTypeVideo) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PLVVideoPreviewCell" forIndexPath:indexPath];
        PLVVideoPreviewCell *currentCell = (PLVVideoPreviewCell *)cell;
    ...
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && model.type == PLVAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PLVGifPreviewCell" forIndexPath:indexPath];
        PLVGifPreviewCell *currentCell = (PLVGifPreviewCell *)cell;
        currentCell.previewView.iCloudSyncFailedHandle = ^(id asset, BOOL isSyncFailed) {
            model.iCloudFailed = isSyncFailed;
            [weakSelf didICloudSyncStatusChanged:model];
        };
    } else {
        cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PLVPhotoPreviewCell" forIndexPath:indexPath];
        PLVPhotoPreviewCell *photoPreviewCell = (PLVPhotoPreviewCell *)cell;
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[PLVPhotoPreviewCell class]]) {
        [(PLVPhotoPreviewCell *)cell recoverSubviews];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didEndDisplayingCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[PLVPhotoPreviewCell class]]) {
        [(PLVPhotoPreviewCell *)cell recoverSubviews];
    } else if ([cell isKindOfClass:[PLVVideoPreviewCell class]]) {
        PLVVideoPreviewCell *videoCell = (PLVVideoPreviewCell *)cell;
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    // 如果正在预览的是视频，隐藏原图按钮
    if (!_isHideNaviBar) {
        if (model.type == PLVAssetModelMediaTypeVideo) {
    ...
    if (![[PLVImageManager manager] isPhotoSelectableWithAsset:model.asset]) {
...
- (void)didICloudSyncStatusChanged:(PLVAssetModel *)model{
    dispatch_async(dispatch_get_main_queue(), ^{
        PLVImagePickerController *_tzImagePickerVc = (PLVImagePickerController *)self.navigationController;
        // onlyReturnAsset为NO时,依赖PLV返回大图,所以需要有iCloud同步失败的提示,并且不能选择,
        if (_tzImagePickerVc.onlyReturnAsset) {
            return;
        }
        PLVAssetModel *currentModel = self.models[self.currentIndex];
...
- (void)showPhotoBytes {
    [[PLVImageManager manager] getPhotosBytesWithArray:@[_models[self.currentIndex]] completion:^(NSString *totalBytes) {
        self->_originalPhotoLabel.text = [NSString stringWithFormat:@"(%@)",totalBytes];
    }];
}

- (NSInteger)currentIndex {
    return [PLVCommonTools tz_isRightToLeftLayout] ? self.models.count - _currentIndex - 1 : _currentIndex;
}
