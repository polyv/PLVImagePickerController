...
@implementation PLVAssetCell

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reload:) name:@"PLV_PHOTO_PICKER_RELOAD_NOTIFICATION" object:nil];
    return self;
}
...
- (void)setModel:(PLVAssetModel *)model {
    ...
    int32_t imageRequestID = [[PLVImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = [[PLVImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
- (void)setType:(PLVAssetCellType)type {
    _type = type;
    if (type == PLVAssetCellTypePhoto || type == PLVAssetCellTypeLivePhoto || (type == PLVAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    } else if (type == PLVAssetCellTypePhotoGif && self.allowPickingGif) {
        self.bottomView.hidden = NO;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:PLVOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self requestBigImage];
    } else { // 取消选中，取消大图的获取
        [self cancelBigImageRequest];
...
- (void)requestBigImage {
    ...
    _bigImageRequestID = [[PLVImageManager manager] requestImageDataForAsset:_model.asset completion:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
        BOOL iCloudSyncFailed = !imageData && [PLVCommonTools isICloudSyncError:info[PHImageErrorKey]];
    ...
    if (_model.type == PLVAssetCellTypeVideo) {
        [[PLVImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
...
- (void)reload:(NSNotification *)noti {
    PLVImagePickerController *tzImagePickerVc = (PLVImagePickerController *)noti.object;
...
@interface PLVAlbumCell ()
@property (weak, nonatomic) UIImageView *posterImageView;
@property (weak, nonatomic) UILabel *titleLabel;
@end

@implementation PLVAlbumCell
...
- (void)setModel:(PLVAlbumModel *)model {
    _model = model;
    ...
    [[PLVImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
@implementation PLVAssetCameraCell
