...
@interface PLVVideoCropController ()<PLVVideoEditViewDelegate,UICollectionViewDelegate, UICollectionViewDataSource> {
    AVPlayer *_player;
...
@property (strong, nonatomic) PLVVideoEditView *videoEditView;
...
@implementation PLVVideoCropController
...
- (void)configMoviePlayer {
    [[PLVImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (void)configVideoImageCollectionView {
    ...
    [_collectionView registerClass:PLVVideoPictureCell.class forCellWithReuseIdentifier:@"PLVVideoPictureCell"];
    [self.view addSubview:_collectionView];
}

- (void)configVideoEditView {
    _videoEditView = PLVVideoEditView.new;
...
- (UIStatusBarStyle)preferredStatusBarStyle {
    if (self.imagePickerVc && [self.imagePickerVc isKindOfClass:[PLVImagePickerController class]]) {
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    PLVVideoPictureCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PLVVideoPictureCell" forIndexPath:indexPath];
...
#pragma mark - PLVVideoEditViewDelegate
...
- (void)doneButtonClick {
    if ([[PLVImageManager manager] isAssetCannotBeSelected:_model.asset]) {
        return;
    }
    [self stopTimer];
    
    PLVImagePickerController *imagePickerVc = self.imagePickerVc;
    [imagePickerVc showProgressHUD];
    [[PLVImageManager manager] getVideoOutputPathWithAsset:_model.asset presetName:imagePickerVc.presetName timeRange:[self getCropVideoTimeRange] success:^(NSString *outputPath) {
...
- (void)callDelegateMethod {
    if (_outputPath) {
        ...
        if (self.imagePickerVc.saveEditedVideoToAlbum) {
            [[PLVImageManager manager] saveVideoWithUrl:videoURL completion:^(PHAsset *asset, NSError *error) {
                if (error) { // 视频保存失败
        ...
        UIImage *coverImage = [[PLVImageManager manager] getImageWithVideoURL:videoURL];
...
- (UIView *)iCloudErrorView{
    if (!_iCloudErrorView) {
        _iCloudErrorView = [[UIView alloc] initWithFrame:CGRectMake(0, [PLVCommonTools tz_statusBarHeight] + 44 + 10, self.view.tz_width, 28)];
...
@implementation PLVVideoEditView {
...
@implementation PLVVideoPictureCell
