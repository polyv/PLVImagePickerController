...
#import "PLVPhotoPickerController.h"
#import "PLVImagePickerController.h"
#import "PLVPhotoPreviewController.h"
#import "PLVAssetCell.h"
#import "PLVAssetModel.h"
#import "UIView+PLVLayout.h"
#import "PLVImageManager.h"
#import "PLVVideoPlayerController.h"
#import "PLVGifPhotoPreviewController.h"
#import <MobileCoreServices/MobileCoreServices.h>
#import "PLVImageRequestOperation.h"

@interface PLVPhotoPickerController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate, PHPhotoLibraryChangeObserver> {
...
@property (nonatomic, strong) PLVCollectionView *collectionView;
...
@implementation PLVPhotoPickerController
...
- (UIImagePickerController *)imagePickerVc {
    if (_imagePickerVc == nil) {
        ...
        if (@available(iOS 9, *)) {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[PLVImagePickerController class]]];
            BarItem = [UIBarButtonItem appearanceWhenContainedInInstancesOfClasses:@[[UIImagePickerController class]]];
        } else {
            tzBarItem = [UIBarButtonItem appearanceWhenContainedIn:[PLVImagePickerController class], nil];
...
- (void)viewDidLoad {
    ...
    [PLVCommonTools configBarButtonItem:cancelItem tzImagePickerVc:tzImagePickerVc];
    ...
    if (tzImagePickerVc.navLeftBarButtonSettingBlock) {
        ...
        [PLVCommonTools configBarButtonItem:backItem tzImagePickerVc:tzImagePickerVc];
...
- (void)fetchAssetModels {
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (!tzImagePickerVc.sortAscendingByModificationDate && self->_isFirstAppear && self->_model.isCameraRoll) {
            [[PLVImageManager manager] getCameraRollAlbumWithFetchAssets:YES completion:^(PLVAlbumModel *model) {
                self->_model = model;
                self->_models = [NSMutableArray arrayWithArray:self->_model.models];
                [self initSubviews];
            }];
        } else if (self->_showTakePhotoBtn || self->_isFirstAppear || !self.model.models || systemVersion >= 14.0) {
            [[PLVImageManager manager] getAssetsFromFetchResult:self->_model.result completion:^(NSArray<PLVAssetModel *> *models) {
...
- (void)configCollectionView {
    if (!_collectionView) {
        ...
        _collectionView = [[PLVCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_layout];
        if (@available(iOS 13.0, *)) {
        ...
        [_collectionView registerClass:[PLVAssetCell class] forCellWithReuseIdentifier:@"PLVAssetCell"];
        [_collectionView registerClass:[PLVAssetCameraCell class] forCellWithReuseIdentifier:@"PLVAssetCameraCell"];
    } else {
...
- (void)configBottomToolBar {
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, [PLVCommonTools tz_isRightToLeftLayout] ? 10 : -10, 0, 0);
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = 50 + [PLVCommonTools tz_safeAreaInsets].bottom;
    if (self.navigationController.navigationBar.isTranslucent) {
        top = naviBarHeight;
        if (!isStatusBarHidden && isFullScreen) top += [PLVCommonTools tz_statusBarHeight];
    ...
    if (!self.navigationController.navigationBar.isHidden) {
        ...
        CGFloat navigationHeight = naviBarHeight + [PLVCommonTools tz_statusBarHeight];
    ...
    [PLVImageManager manager].columnNumber = [PLVImageManager manager].columnNumber;
    [PLVImageManager manager].photoWidth = tzImagePickerVc.photoWidth;
...
- (void)previewButtonClick {
    PLVPhotoPreviewController *photoPreviewVc = [[PLVPhotoPreviewController alloc] init];
...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.onlyReturnAsset) { // not fetch image
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            PLVAssetModel *model = tzImagePickerVc.selectedModels[i];
        ...
        [PLVImageManager manager].shouldFixOrientation = YES;
        __block UIAlertController *alertView;
        for (NSInteger i = 0; i < tzImagePickerVc.selectedModels.count; i++) {
            PLVAssetModel *model = tzImagePickerVc.selectedModels[i];
            PLVImageRequestOperation *operation = [[PLVImageRequestOperation alloc] initWithAsset:model.asset completion:^(UIImage * _Nonnull photo, NSDictionary * _Nonnull info, BOOL isDegraded) {
                if (isDegraded) return;
                if (photo) {
                    if (![PLVImagePickerConfig sharedInstance].notScaleImage) {
                        photo = [[PLVImageManager manager] scaleImage:photo toSize:CGSizeMake(tzImagePickerVc.photoWidth, (int)(tzImagePickerVc.photoWidth * photo.size.height / photo.size.width))];
                    }
...
- (void)callDelegateMethodWithPhotos:(NSArray *)photos assets:(NSArray *)assets infoArr:(NSArray *)infoArr {
    ...
    if (tzImagePickerVc.allowPickingVideo && tzImagePickerVc.maxImagesCount == 1) {
        if ([[PLVImageManager manager] isVideo:[assets firstObject]]) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (((tzImagePickerVc.sortAscendingByModificationDate && indexPath.item >= _models.count) || (!tzImagePickerVc.sortAscendingByModificationDate && indexPath.item == 0)) && _showTakePhotoBtn) {
        PLVAssetCameraCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PLVAssetCameraCell" forIndexPath:indexPath];
    ...
    PLVAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"PLVAssetCell" forIndexPath:indexPath];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        PLVImagePickerController *tzImagePickerVc = (PLVImagePickerController *)strongSelf.navigationController;
        // 1. cancel select / 取消选择
        if (isSelected) {
            strongCell.selectPhotoButton.selected = NO;
            model.isSelected = NO;
            NSArray *selectedModels = [NSArray arrayWithArray:tzImagePickerVc.selectedModels];
            for (PLVAssetModel *model_item in selectedModels) {
            ...
            if (tzImagePickerVc.showSelectedIndex || tzImagePickerVc.showPhotoCannotSelectLayer) {
                [[NSNotificationCenter defaultCenter] postNotificationName:@"PLV_PHOTO_PICKER_RELOAD_NOTIFICATION" object:strongSelf.navigationController];
            }
            [UIView showOscillatoryAnimationWithLayer:strongLayer type:PLVOscillatoryAnimationToSmaller];
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                if ([[PLVImageManager manager] isAssetCannotBeSelected:model.asset]) {
                    return;
                }
                if (!tzImagePickerVc.allowPreview) {
                    BOOL shouldDone = tzImagePickerVc.maxImagesCount == 1;
                    if (!tzImagePickerVc.allowPickingMultipleVideo && (model.type == PLVAssetModelMediaTypeVideo || model.type == PLVAssetModelMediaTypePhotoGif)) {
                        shouldDone = YES;
                    }
                    if (shouldDone) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PLVAssetModel *model = _models[index];
    if (model.type == PLVAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            ...
            PLVVideoPlayerController *videoPlayerVc = [[PLVVideoPlayerController alloc] init];
            videoPlayerVc.model = model;
            [self.navigationController pushViewController:videoPlayerVc animated:YES];
        }
    } else if (model.type == PLVAssetModelMediaTypePhotoGif && tzImagePickerVc.allowPickingGif && !tzImagePickerVc.allowPickingMultipleVideo) {
        if (tzImagePickerVc.selectedModels.count > 0) {
            PLVImagePickerController *imagePickerVc = (PLVImagePickerController *)self.navigationController;
            [imagePickerVc showAlertWithTitle:[NSBundle tz_localizedStringForKey:@"Can not choose both photo and GIF"]];
        } else {
            PLVGifPhotoPreviewController *gifPreviewVc = [[PLVGifPhotoPreviewController alloc] init];
...
- (void)takePhoto {
    ...
    if ((authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)) {
        ...
        NSDictionary *infoDict = [PLVCommonTools tz_getInfoDictionary];
        // 无权限 做一个友好的提示
...
- (void)pushPhotoPrevireViewController:(PLVPhotoPreviewController *)photoPreviewVc {
    [self pushPhotoPrevireViewController:photoPreviewVc needCheckSelectedModels:NO];
}

- (void)pushPhotoPrevireViewController:(PLVPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    __weak typeof(self) weakSelf = self;
...
- (void)getSelectedPhotoBytes {
    ...
    if ([[PLVImagePickerConfig sharedInstance].preferredLanguage isEqualToString:@"vi"] && self.view.tz_width <= 320) {
        return;
    }
    PLVImagePickerController *imagePickerVc = (PLVImagePickerController *)self.navigationController;
    [[PLVImageManager manager] getPhotosBytesWithArray:imagePickerVc.selectedModels completion:^(NSString *totalBytes) {
...
- (void)checkSelectedModels {
    ...
    for (PLVAssetModel *model in selectedModels) {
...
- (void)callDelegate:(PHAsset *)asset isSelect:(BOOL)isSelect {
    ...
    [[PLVImageManager manager] getPhotoWithAsset:asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (isDegraded) return;
        __strong typeof(weakSelf) strongSelf = weakSelf;
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:@"public.image"]) {
        ...
        if (photo) {
            ...
            [[PLVImageManager manager] savePhotoWithImage:photo meta:meta location:self.location completion:^(PHAsset *asset, NSError *error){
        ...
        if (videoUrl) {
            self.isSavingMedia = YES;
            [[PLVImageManager manager] saveVideoWithUrl:videoUrl location:self.location completion:^(PHAsset *asset, NSError *error) {
                self.isSavingMedia = NO;
                if (!error && asset) {
                    [self addPHAsset:asset];
                } else {
...
- (void)addPHAsset:(PHAsset *)asset {
    PLVAssetModel *assetModel = [[PLVImageManager manager] createModelWithAsset:asset];
    PLVImagePickerController *tzImagePickerVc = (PLVImagePickerController *)self.navigationController;
    [tzImagePickerVc hideProgressHUD];
    if (tzImagePickerVc.sortAscendingByModificationDate) {
    ...
    if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
        if (assetModel.type == PLVAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo) {
            // 不能多选视频的情况下，不选中拍摄的视频
        } else {
            if ([[PLVImageManager manager] isAssetCannotBeSelected:assetModel.asset]) {
                return;
            }
...
- (void)resetCachedAssets {
    [[PLVImageManager manager].cachingImageManager stopCachingImagesForAllAssets];
...
- (void)updateCachedAssets {
    ...
    if (delta > CGRectGetHeight(_collectionView.bounds) / 3.0f) {
        ...
        [[PLVImageManager manager].cachingImageManager startCachingImagesForAssets:assetsToStartCaching
                                                                       targetSize:AssetGridThumbnailSize
                                                                      contentMode:PHImageContentModeAspectFill
                                                                          options:nil];
        [[PLVImageManager manager].cachingImageManager stopCachingImagesForAssets:assetsToStopCaching
                                                                      targetSize:AssetGridThumbnailSize
                                                                     contentMode:PHImageContentModeAspectFill
                                                                         options:nil];
...
- (NSArray *)assetsAtIndexPaths:(NSArray *)indexPaths {
    ...
    for (NSIndexPath *indexPath in indexPaths) {
        if (indexPath.item < _models.count) {
            PLVAssetModel *model = _models[indexPath.item];
            [assets addObject:model.asset];
        }
    }
...
@implementation PLVCollectionView
